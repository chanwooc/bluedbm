...
//	for (int lpa = lpaStart; lpa < numLPA; lpa++) {
//		if (counter++ >= 2) {
//			usleep(300);
//			counter = 0;
//		}
...
//		if (elapsed == 0) {
//			elapsed=10000;
//			printf("numWritesInFlight = %d\n", getNumWritesInFlight()); fflush(stdout);
//		}
//		else {
//			elapsed--;
//		}
...
//		}
	...
	while (true) {
		...
		if (elapsed == 0) {
			elapsed=10000;
			printf("numReadsInFlight = %d\n", getNumReadsInFlight()); fflush(stdout);
		}
		else {
			elapsed--;
		}
	...
	clock_gettime(CLOCK_REALTIME, & now);
	fprintf(stderr, "LOG: finished reading! %f\n", timespec_diff_sec(start, now) );
	fprintf(stderr, "SPEED: %f MB/s\n", (8192.0*numLPA/1000000)/timespec_diff_sec(start,now));
